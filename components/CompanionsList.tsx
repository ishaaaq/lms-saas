...
export default function CompanionsList({title, companions, classNames}: CompanionsListProps) {
 ...
 <TableHeader>
  ...
  <TableBody>
    {companions?.map(({id, name, subject, topic, duration, color}) => (
        <TableRow key={id}>
            <TableCell>
                <Link href={`/companions/${id}`} className="font-medium">
              <div className="flex items-center gap-2">
                <div style={{backgroundColor: color}} className={`size-[72px]  max-md:hidden rounded-lg flex items-center justify-center`}>
         ...
         {subject}
            ...
            <TableCell>
<div className=" text-md font-semibold gqp-2 w-full flex items-center justify-end">
